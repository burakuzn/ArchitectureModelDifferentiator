...
operation Any extract():String {
	...
	}else if(self.eClass().name == "Implementation"){
		return self.child.name +" implements " + self.parent.name;
	}else if(self.eClass().name == "Inheritance"){
		return self.child.name +" inherits " + self.parent.name;
	}
	return "unknown";
}